...
export async function POST({ cookies }: APIContext) {
      ...
      console.log("[AI] Generating content for each device..");
      for(let i = 0; i < devices_data.length; i++) {
            // device ID. Will be used to get the sensors data
            const target_device_id = devices_data[i].id;
            ...
            if(Object.values(sensors_data).some((data) => data.length == 0)) {
                  console.log(`[AI] Generating content.. (ID: ${target_device_id}) [${i + 1}/${devices_data.length}] (skipped)`);
                  continue;
            };
            ...
            console.log(`[AI] Generating content.. (ID: ${target_device_id}) [${i + 1}/${devices_data.length}]`);
      ...
      `
            ...
            try {
                  ...
                  console.error(`Uhmmm... I got this error when creating AI reports to the database.. IDK why though :(\n${err}`);
                  return create_response({ status: 500, body: { message: "There's an error" } })
            }
      }
